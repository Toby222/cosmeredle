...
test("Kholin is a family", () => {
	const jasnah = characters.find((character) =>
		character.name.includes("Jasnah"),
	);
	expect(jasnah).not.toBeUndefined();
	const renarin = characters.find((character) =>
		character.name.includes("Renarin"),
	);
...
test("Proper book occurence count", () => {
	const books = characters.map((character) => character.firstAppearance);
